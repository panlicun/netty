...
public class NettyTransferClientHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelActive(ChannelHandlerContext ctx) {
        try {
            ...
            String[] clientAddr = secure.getClientAddr().split(":");
            secure.setOriginIp(clientAddr[0]);
            secure.setOriginPort(Integer.parseInt(clientAddr[1]));
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof SecureModel) {
            ...
            if (secureModel.isAutoSuccess()) {
                log.info("连接" + ultimateIp + ":" + ultimatePort + "通道建立成功，可以进行传输...");
