		...
		if (msg instanceof RequestFile) {
			...
			if(start == 0){ //只有在文件开始传的时候才进入 这样就减少了对象创建 和可能出现的一些错误
				...
				if( file.exists() ) {
					...
					responseFile.setOriginIp(ef.getOriginIp());
					responseFile.setOriginPort(ef.getOriginPort());
			...
			if (byteRead > 0 && (start < fileSize && fileSize != -1)) {
				...
				ResponseFile responseFile = new ResponseFile(start,md5,(start*100)/fileSize,ef);
				responseFile.setOriginIp(ef.getOriginIp());
				responseFile.setOriginPort(ef.getOriginPort());
				ctx.writeAndFlush(responseFile);
			} else {
